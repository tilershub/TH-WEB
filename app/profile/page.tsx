...
import type { Profile, Role, PortfolioItem, Certification } from "@/lib/types";
    ...
    <RequireAuth>
      <div className="min-h-screen bg-gray-100 pb-28">
        <div className="max-w-lg mx-auto bg-white min-h-screen shadow-lg">
          <div className="relative">
            ...
            <div className="px-5 pb-4 -mt-10">
              <div className="flex items-end gap-4">
                <div className="w-20 h-20 rounded-full border-4 border-white bg-gradient-to-br from-primary to-primary-dark overflow-hidden flex-shrink-0 shadow-lg">
                  {avatarUrl ? (
                    <img src={avatarUrl} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-white text-2xl font-bold">
          ...
          {skillsWithRates.length > 0 ? (
            <>
              <div className="px-5 py-5">
                <h2 className="text-lg font-bold text-gray-900 mb-4">My Services & Rates</h2>
                <div className="space-y-3">
                  {skillsWithRates.map((skill) => (
                    <div key={skill.label} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      {skill.rate ? (
                        <span className="text-primary font-semibold text-sm">
                          Rs. {skill.rate.toLocaleString()} <span className="text-gray-500 font-normal text-xs">/ {skill.unit.replace('LKR/', '')}</span>
                        </span>
                      ) : (
            ...
            <>
              <div className="px-5 py-5">
                <h2 className="text-lg font-bold text-gray-900 mb-3">My Services</h2>
                <p className="text-gray-500 text-sm">No services added yet. Complete your profile to add services.</p>
              </div>
              <div className="border-t border-gray-100" />
            </>
          )}
              ...
              <div className="px-5 py-5">
                ...
                <div className="space-y-3">
                  {certifications.map((cert) => {
                    const certImgUrl = getPublicUrl("certifications", cert.image_path);
                    return (
          ...
          <div className="px-5 py-5">
            ...
            {profile?.id && (
              <Link
                ...
                className="w-full flex items-center justify-center gap-2 border border-primary text-primary font-medium py-3 px-6 rounded-xl hover:bg-primary/5 transition-colors"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
